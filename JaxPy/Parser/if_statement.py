        ...
        if match_terminal(ELSE):
            if match_terminal(OPENING_BRACE, True, Scope_Type.IF):
                if parser.MST():
                    if match_terminal(CLOSING_BRACE):
                        return True
    elif select_rule([LET_CONST, IF, UNTIL, PROCEDURE, RETURN, ASSIGN, BREAK_CONTINUE, THIS, IDENTIFIER, INTEGER_CONSTANT, FLOAT_CONSTANT, STRING_CONSTANT, BOOL_CONSTANT, NOT, FINAL, CLASS, INTERFACE, END_MARKER, CLOSING_BRACE]):
        return True
    return False